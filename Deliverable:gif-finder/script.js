...
//Now, below is a function called sendApiRequest(). Call that function in the above button click event with searchInput as it's argument
...
function sendApiRequest(x) {
  //Where Ryan+Gosling is located replace it with x
    fetch("https://api.giphy.com/v1/gifs/search?api_key=ML1LkNEzCziuzCUQR9OgSGKZeghKl6cK&q="+ x +"&limit=25&offset=0&rating=G&lang=en")
    .then(function(data) {
      return data.json();
    })
    .then(function(json) {
...
function getImageURLfrom(myJSON) {
  ...
  const imageURL = myJSON.data[Math.floor(Math.random() * myJSON.data.length)].images.original.url
...
function addImageToScreen(myURL) {
  ...
  //Next call the .innerHTML method on your wrapper varible and set it equal to the following string 
